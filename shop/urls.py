from django.urls import path

from .views import (
    AccountCenterView,
    AnnouncementDetailView,
    CheckoutView,
    CreateOrderView,
    GuestOrderLookupView,
    HealthView,
    HelpArticleDetailView,
    HelpCenterView,
    MerchantDashboardView,
    MerchantInventoryCodeRevealView,
    MerchantInventoryView,
    MerchantInventoryBatchActionView,
    MerchantOrderActionView,
    MerchantOrderDetailView,
    MerchantOrderListView,
    MerchantUserDetailView,
    MerchantUserListView,
    MerchantProductCreateView,
    MerchantProductBatchActionView,
    DeliveryRecordRevealView,
    MerchantProductListView,
    MerchantSupportTicketDetailView,
    MerchantSupportTicketListView,
    MerchantProductToggleStatusView,
    MerchantProductUpdateView,
    MockPaymentView,
    OrderDetailView,
    PaymentCancelView,
    PaymentSuccessView,
    ProductDetailView,
    ReadinessView,
    ReorderView,
    StartPaymentView,
    StorefrontView,
    SupportView,
    SupportTicketDetailView,
    StripeWebhookView,
)

urlpatterns = [
    path("", StorefrontView.as_view(), name="storefront"),
    path("health/", HealthView.as_view(), name="health"),
    path("health/readiness/", ReadinessView.as_view(), name="readiness"),
    path("announcements/<int:pk>/", AnnouncementDetailView.as_view(), name="announcement_detail"),
    path("help/", HelpCenterView.as_view(), name="help_center"),
    path("help/<slug:slug>/", HelpArticleDetailView.as_view(), name="help_article_detail"),
    path("order-lookup/", GuestOrderLookupView.as_view(), name="order_lookup"),
    path("support/", SupportView.as_view(), name="support"),
    path("support/tickets/<slug:ticket_no>/", SupportTicketDetailView.as_view(), name="support_ticket_detail"),
    path("dashboard/", MerchantDashboardView.as_view(), name="merchant_dashboard"),
    path("dashboard/products/", MerchantProductListView.as_view(), name="merchant_products"),
    path("dashboard/products/actions/", MerchantProductBatchActionView.as_view(), name="merchant_product_batch_action"),
    path("dashboard/products/new/", MerchantProductCreateView.as_view(), name="merchant_product_create"),
    path("dashboard/products/<int:pk>/edit/", MerchantProductUpdateView.as_view(), name="merchant_product_edit"),
    path("dashboard/products/<int:pk>/toggle/", MerchantProductToggleStatusView.as_view(), name="merchant_product_toggle"),
    path("dashboard/inventory/", MerchantInventoryView.as_view(), name="merchant_inventory"),
    path("dashboard/inventory/actions/", MerchantInventoryBatchActionView.as_view(), name="merchant_inventory_batch_action"),
    path("dashboard/inventory/codes/<int:pk>/reveal/", MerchantInventoryCodeRevealView.as_view(), name="merchant_inventory_code_reveal"),
    path("dashboard/orders/", MerchantOrderListView.as_view(), name="merchant_orders"),
    path("dashboard/orders/<slug:order_no>/actions/", MerchantOrderActionView.as_view(), name="merchant_order_action"),
    path("dashboard/orders/<slug:order_no>/", MerchantOrderDetailView.as_view(), name="merchant_order_detail"),
    path("dashboard/users/", MerchantUserListView.as_view(), name="merchant_users"),
    path("dashboard/users/<int:pk>/", MerchantUserDetailView.as_view(), name="merchant_user_detail"),
    path("dashboard/support/", MerchantSupportTicketListView.as_view(), name="merchant_support_tickets"),
    path("dashboard/support/<slug:ticket_no>/", MerchantSupportTicketDetailView.as_view(), name="merchant_support_ticket_detail"),
    path("me/", AccountCenterView.as_view(), name="account_center"),
    path("products/<slug:slug>/", ProductDetailView.as_view(), name="product_detail"),
    path("products/<slug:slug>/buy/", CreateOrderView.as_view(), name="create_order"),
    path("orders/<slug:order_no>/checkout/", CheckoutView.as_view(), name="checkout"),
    path("orders/<slug:order_no>/pay/", StartPaymentView.as_view(), name="start_payment"),
    path("orders/<slug:order_no>/mock-pay/", MockPaymentView.as_view(), name="mock_pay"),
    path("orders/<slug:order_no>/reorder/", ReorderView.as_view(), name="reorder"),
    path("orders/<slug:order_no>/deliveries/<int:delivery_id>/reveal/", DeliveryRecordRevealView.as_view(), name="delivery_reveal"),
    path("orders/<slug:order_no>/success/", PaymentSuccessView.as_view(), name="payment_success"),
    path("orders/<slug:order_no>/cancel/", PaymentCancelView.as_view(), name="payment_cancel"),
    path("orders/<slug:order_no>/", OrderDetailView.as_view(), name="order_detail"),
    path("webhooks/stripe/", StripeWebhookView.as_view(), name="stripe_webhook"),
]
